...
for ii,(tno,tkm,tname) in enumerate(zip(nos,kms,noaa.row_names[nind])):
    ...
    ax[ii].legend([link,linn], 
                  ["cluster {0}".format(cind[ii]+1),tname[0].replace("_"," ")],
                  frameon=False)
...
########################
#
## -- load the data and generate mean bright sources spectra
#waves    = np.load("../output/vnir_waves.npy")*1e-3
#spec_tot = np.load("../output/clean_spec_tot.npy")*1600.*1560.
#
## -- make the plot
#fig, ax = plt.subplots(figsize=[6.5,3.25])
#fig.subplots_adjust(0.085,0.125,0.95,0.87)
#ax.grid("on")
#lint, = ax.plot(waves,spec_tot/spec_tot.max()*0.9,color="darkred")
#yr = [-0.1,1.0]
#ax.set_ylim(yr)
#ax.plot([0.570,0.570],yr,color="darkorange")
#ax.plot([0.595,0.595],yr,color="darkorange")
#ax.plot([0.818,0.818],yr,color="darkorange")
#ax.plot([0.547,0.547],yr,color="darkorange")
#ax.plot([0.612,0.612],yr,color="darkorange")
#
#ax.text(0.570,yr[1]+0.02*(yr[1]-yr[0]),"Na-II",fontsize=txtsz,ha="center",
#        rotation=90,va="bottom")
#ax.text(0.595,yr[1]+0.02*(yr[1]-yr[0]),"Na-II",fontsize=txtsz,ha="center",
#        rotation=90,va="bottom")
#ax.text(0.818,yr[1]+0.02*(yr[1]-yr[0]),"Na-I",fontsize=txtsz,ha="center",
#        rotation=90,va="bottom")
#ax.text(0.547,yr[1]+0.02*(yr[1]-yr[0]),"Hg",fontsize=txtsz,ha="center",
#        rotation=90,va="bottom")
#ax.text(0.612,yr[1]+0.02*(yr[1]-yr[0]),"Eu-III",fontsize=txtsz,ha="center",
#        rotation=90,va="bottom")
#
...
#fig.canvas.draw()
#plt.show()
#fig.savefig("../output/total_spectrum.eps", clobber=True)
